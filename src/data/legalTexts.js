// Textes légaux d'Apézeo — générés à partir des documents rédigés.
// Remplacer les [À COMPLÉTER] par vos informations réelles avant publication.

export const MENTIONS_LEGALES = `# Mentions légales — Apézeo

*Document préparé comme base de travail. Les champs entre crochets [ ] doivent être complétés avec vos informations exactes avant publication. Une relecture par un avocat spécialisé en droit du numérique est recommandée avant mise en ligne commerciale.*

---

## Éditeur du site

Le site et l'application Apézeo (ci-après « Apézeo » ou « le Service ») sont édités par :

- **Raison sociale / Nom** : [À COMPLÉTER — ex. Apézeo SAS / votre nom si vous exercez en nom propre]
- **Forme juridique** : [À COMPLÉTER — SAS, SASU, EI, association, etc.]
- **Capital social** : [À COMPLÉTER, si applicable]
- **Siège social** : [À COMPLÉTER — adresse complète]
- **Numéro SIREN/SIRET** : [À COMPLÉTER]
- **Numéro de TVA intracommunautaire** : [À COMPLÉTER, si applicable]
- **Directeur de la publication** : [À COMPLÉTER — nom du responsable]
- **Contact** : contact@apezeo.fr

## Hébergement

### Hébergement de l'application
L'application est hébergée par :
**Vercel Inc.**
440 N Barranca Ave #4133, Covina, CA 91723, États-Unis
[https://vercel.com](https://vercel.com)

### Hébergement de la base de données
Les données sont hébergées par :
**Supabase Inc.**
[À COMPLÉTER — vérifier l'adresse du siège Supabase et la région de données choisie lors de la création du projet, ex. UE/Irlande ou États-Unis]
[https://supabase.com](https://supabase.com)

*Point d'attention : si l'hébergement des données Supabase n'est pas localisé dans l'Union européenne, un transfert de données hors UE a lieu — voir la section correspondante de la politique de confidentialité. Il est recommandé de vérifier/configurer la région d'hébergement Supabase sur un serveur européen si cela n'a pas déjà été fait.*

## Propriété intellectuelle

L'ensemble des éléments constituant Apézeo (structure, textes, bibliothèque de fiches, base de données, logo, charte graphique, code source) est protégé par le droit de la propriété intellectuelle. Toute reproduction, représentation, modification, publication ou adaptation, totale ou partielle, sans l'autorisation écrite préalable de l'éditeur, est interdite, sauf pour l'usage strictement personnel prévu par les présentes conditions d'utilisation.

Le contenu des fiches soumis par les utilisateurs via la fonctionnalité « Soumettre publiquement » demeure sous réserve des droits de son auteur jusqu'à validation et intégration explicite par l'éditeur (voir CGU, article dédié).

## Objet du Service

Apézeo est un outil d'information et de formation continue destiné aux professionnels accompagnant des personnes atteintes de la maladie d'Alzheimer ou de maladies apparentées. **Apézeo n'est pas un dispositif médical et ne fournit aucun diagnostic, prescription ou avis médical individualisé** (voir la Clause de non-responsabilité, qui fait partie intégrante des présentes mentions légales et des CGU).

## Droit applicable

Les présentes mentions légales sont soumises au droit français. En cas de litige, et à défaut de résolution amiable, les tribunaux français seront seuls compétents.

## Contact

Pour toute question relative aux présentes mentions légales : **contact@apezeo.fr**

---

*Dernière mise à jour : [À COMPLÉTER — date de publication]*
`;

export const CGU = `# Conditions Générales d'Utilisation (CGU) — Apézeo

*Document préparé comme base de travail, à faire relire par un avocat spécialisé en droit du numérique avant publication et avant toute signature de contrat B2B.*

---

## Article 1 — Objet

Les présentes Conditions Générales d'Utilisation (CGU) régissent l'accès et l'utilisation de l'application Apézeo (ci-après « le Service »), éditée par [À COMPLÉTER — voir Mentions légales], destinée aux professionnels accompagnant des personnes atteintes de la maladie d'Alzheimer et de maladies apparentées (« Version Pro »).

L'utilisation du Service implique l'acceptation pleine et entière des présentes CGU, ainsi que de la [Politique de confidentialité](#) et de la [Clause de non-responsabilité](#), qui en font partie intégrante.

## Article 2 — Accès au Service

### 2.1 Version Pro
L'accès à la Version Pro est réservé aux professionnels accompagnant des personnes atteintes de la maladie d'Alzheimer ou de maladies apparentées (secteur sanitaire, social et médico-social). L'inscription requiert la création d'un compte (adresse e-mail, mot de passe, profession déclarée).

### 2.2 Comptes rattachés à une structure
Un compte peut être rattaché à un établissement (« la Structure ») au moyen d'un code d'invitation fourni par l'administrateur de cette Structure. Ce rattachement donne accès à la bibliothèque complète de fiches, dans la limite du quota de comptes souscrit par la Structure.

### 2.3 Compte sans structure (accès découverte)
Un compte créé sans code d'invitation bénéficie d'un accès limité à un échantillon de la bibliothèque, présenté à titre de découverte du Service.

### 2.4 Version Aidant
Une version simplifiée, accessible sans compte, est proposée aux aidants familiaux. Elle n'est pas soumise aux dispositions spécifiques aux comptes professionnels des présentes CGU.

## Article 3 — Obligations de l'utilisateur

L'utilisateur s'engage à :

- fournir des informations exactes lors de son inscription,
- **ne jamais partager ses identifiants de connexion avec des personnes non autorisées**, y compris en dehors de sa Structure. Le partage d'identifiants constitue un manquement pouvant entraîner la suspension du compte,
- ne pas tenter de contourner les limitations d'accès ou les quotas de comptes attachés à sa Structure,
- **ne jamais saisir de données identifiantes concernant une personne accompagnée** (nom, prénom, éléments permettant une identification indirecte, diagnostic précis) dans les champs de texte libre de l'application (notamment le champ « commentaire » de l'historique d'usage). Voir également la Politique de confidentialité, article relatif aux données sensibles,
- utiliser le Service conformément à son objet, tel que défini par la Clause de non-responsabilité, et notamment à ne jamais substituer le contenu d'Apézeo à un avis médical ou à l'intervention des services d'urgence lorsque la situation l'exige.

## Article 4 — Contenu soumis par les utilisateurs

### 4.1 Fiches personnelles
Chaque utilisateur peut créer des fiches personnelles, visibles uniquement par lui-même (Version Pro) ou stockées localement sur son appareil (Version Aidant). Ce contenu reste sous la responsabilité de son auteur.

### 4.2 Soumission publique
L'utilisateur peut soumettre une fiche personnelle à l'éditeur en vue d'une éventuelle intégration à la bibliothèque partagée, via la fonctionnalité « Soumettre publiquement ». Cette soumission :

- est étudiée par l'éditeur selon sa pertinence (cohérence entre la réalisation proposée, les objectifs attendus et les moyens à mettre en œuvre),
- n'entraîne aucune intégration automatique à la bibliothèque partagée,
- transfère à l'éditeur, en cas d'intégration, le droit d'utiliser, adapter et diffuser le contenu soumis au sein du Service, l'auteur en conservant la paternité intellectuelle sauf mention contraire.

## Article 5 — Administration d'une Structure

L'administrateur d'une Structure (compte disposant du rôle « admin ») peut :

- consulter la liste des comptes rattachés à sa Structure,
- activer ou désactiver l'accès d'un compte membre de sa Structure.

L'administrateur d'une Structure s'engage à n'utiliser ces fonctionnalités que dans le cadre de la gestion légitime des accès de son établissement, dans le respect du droit du travail et des droits des personnes concernées (voir Politique de confidentialité).

## Article 6 — Abonnement et facturation (Structures)

L'accès à la Version Pro pour une Structure (établissement, groupe d'établissements) fait l'objet d'un **tarif établi sur devis**, adapté au nombre de comptes souhaité et à l'organisation demandeuse. Les modalités d'abonnement, de facturation et de résiliation applicables à une Structure cliente font l'objet d'un contrat commercial distinct entre l'éditeur et la Structure. [À COMPLÉTER — référencer ici votre futur contrat-type / CGV B2B une fois rédigé].

## Article 7 — Disponibilité du Service

L'éditeur s'efforce d'assurer la disponibilité du Service mais ne garantit pas un accès ininterrompu, notamment en cas de maintenance, de panne des prestataires techniques (hébergement, base de données) ou de cas de force majeure. L'éditeur ne saurait être tenu responsable d'une indisponibilité temporaire du Service.

## Article 8 — Propriété intellectuelle

Voir les [Mentions légales](#), article « Propriété intellectuelle ».

## Article 9 — Limitation de responsabilité

Voir la [Clause de non-responsabilité](#), qui fait partie intégrante des présentes CGU.

## Article 10 — Suspension et résiliation

L'éditeur se réserve le droit de suspendre ou de supprimer, sans préavis en cas de manquement grave, tout compte utilisateur ne respectant pas les présentes CGU, notamment en cas de partage d'identifiants constaté, de saisie de données identifiantes concernant une personne accompagnée, ou d'usage détourné du Service.

L'utilisateur peut demander la suppression de son compte à tout moment en écrivant à contact@apezeo.fr.

## Article 11 — Modification des CGU

L'éditeur se réserve le droit de modifier les présentes CGU à tout moment. Les utilisateurs seront informés de toute modification substantielle. La poursuite de l'utilisation du Service après modification vaut acceptation des nouvelles CGU.

## Article 12 — Droit applicable et litiges

Les présentes CGU sont soumises au droit français. Tout litige relatif à leur interprétation ou à leur exécution relève, à défaut de résolution amiable, de la compétence des tribunaux français.

## Article 13 — Contact

Pour toute question relative aux présentes CGU : **contact@apezeo.fr**

---

*Dernière mise à jour : [À COMPLÉTER — date de publication]*
`;

export const CONFIDENTIALITE = `# Politique de confidentialité — Apézeo

*Document préparé comme base de travail, à faire relire par un avocat spécialisé RGPD/droit du numérique avant publication. Ce document ne remplace pas le registre des activités de traitement (article 30 RGPD), document interne distinct que l'éditeur doit également constituer et tenir à jour.*

---

## 1. Responsable de traitement

Le responsable du traitement des données personnelles collectées par Apézeo est :
[À COMPLÉTER — voir Mentions légales]
Contact dédié aux questions de protection des données : **contact@apezeo.fr**

*Point d'attention : selon le volume de comptes et le type de données traitées, la désignation d'un Délégué à la Protection des Données (DPO) pourra devenir nécessaire ou recommandée. À réévaluer avec un conseil RGPD à mesure de la croissance du Service.*

## 2. Données collectées

### 2.1 Compte professionnel (Version Pro)
- Adresse e-mail
- Mot de passe (stocké de façon chiffrée par le prestataire d'authentification, jamais en clair)
- Profession déclarée
- Structure de rattachement (le cas échéant) et rôle (administrateur / membre)
- Statut du compte (actif / désactivé)

### 2.2 Données d'usage
- Favoris (fiches marquées comme utiles)
- Historique des essais réalisés : intensité du trouble avant/après une technique, durée, **commentaire libre**
- Fiches personnelles créées par l'utilisateur

**Point de vigilance spécifique** : le champ « commentaire » de l'historique et le contenu des fiches personnelles sont des champs de texte libre. L'éditeur demande explicitement aux utilisateurs, via les CGU, de ne jamais y saisir d'information permettant d'identifier une personne accompagnée (nom, éléments d'identification indirecte, diagnostic précis). Ces champs ne sont *pas destinés* à contenir des données de santé concernant un tiers, mais l'éditeur ne peut techniquement garantir qu'aucun utilisateur n'y contreviendra. [À COMPLÉTER une fois la stratégie de modération/filtrage éventuelle décidée avec votre conseil].

### 2.3 Version Aidant
La Version Aidant ne requiert pas de compte. Les données saisies (favoris, historique, fiches personnelles) sont conservées **localement sur l'appareil de l'utilisateur uniquement** et ne sont jamais transmises à l'éditeur.

### 2.4 Données techniques
Données de connexion et journaux techniques usuels générés par les prestataires d'hébergement (voir section « Sous-traitants »).

## 3. Finalités du traitement

| Donnée | Finalité | Base légale |
|---|---|---|
| Compte professionnel | Gestion de l'accès au Service, authentification | Exécution du contrat (CGU) |
| Structure / rôle | Gestion des accès et quotas au niveau de l'établissement client | Exécution du contrat (relation B2B avec la Structure) |
| Favoris / historique | Amélioration de l'expérience d'usage, aide à la décision personnalisée | Intérêt légitime de l'utilisateur et de l'éditeur |
| Fiches personnelles | Fonctionnalité du Service | Exécution du contrat |

## 4. Destinataires des données

Les données sont accessibles :
- à l'utilisateur lui-même, pour ses propres données,
- à l'administrateur de sa Structure, pour les informations nécessaires à la gestion des comptes (e-mail, profession, statut actif/inactif) — **jamais l'historique d'usage individuel ni les favoris d'un membre**, sauf évolution future du Service qui serait alors documentée ici,
- à l'éditeur d'Apézeo, dans la limite nécessaire au support et à la maintenance du Service,
- aux sous-traitants techniques listés ci-dessous.

## 5. Sous-traitants et hébergement

| Prestataire | Rôle | Localisation | Transfert hors UE |
|---|---|---|---|
| Supabase Inc. | Base de données et authentification | [À COMPLÉTER — vérifier la région choisie lors de la création du projet] | [À COMPLÉTER — si hors UE, indiquer le mécanisme de transfert : clauses contractuelles types de la Commission européenne, etc.] |
| Vercel Inc. | Hébergement de l'application | États-Unis (infrastructure mondiale) | Oui — clauses contractuelles types applicables selon la documentation Vercel |

*Recommandation forte avant publication : vérifier/configurer la région d'hébergement Supabase sur un serveur situé dans l'Union européenne si ce n'est pas déjà le cas, afin de simplifier la conformité RGPD et réduire les obligations liées aux transferts internationaux de données.*

## 6. Durée de conservation

- **Compte actif** : les données sont conservées pendant toute la durée d'utilisation du Service.
- **Compte désactivé par un administrateur de Structure** : [À COMPLÉTER — définir une durée, ex. conservation 12 mois puis anonymisation/suppression, sauf obligation légale de conservation plus longue].
- **Compte supprimé à la demande de l'utilisateur** : suppression sous [À COMPLÉTER, ex. 30 jours].
- **Version Aidant** : les données restant locales à l'appareil, leur durée de conservation dépend uniquement des actions de l'utilisateur sur son propre appareil.

## 7. Droits des personnes concernées

Conformément au RGPD, tout utilisateur dispose des droits suivants sur ses données personnelles :
- droit d'accès,
- droit de rectification,
- droit à l'effacement (« droit à l'oubli »),
- droit à la limitation du traitement,
- droit à la portabilité,
- droit d'opposition,
- droit de définir des directives relatives au sort de ses données après son décès.

Ces droits peuvent être exercés en écrivant à **contact@apezeo.fr**. Une réponse sera apportée dans le délai légal d'un mois.

En cas de désaccord persistant, l'utilisateur peut introduire une réclamation auprès de la Commission Nationale de l'Informatique et des Libertés (CNIL) — [www.cnil.fr](https://www.cnil.fr).

## 8. Sécurité

L'éditeur met en œuvre les mesures techniques et organisationnelles raisonnables pour protéger les données contre l'accès non autorisé, la perte ou l'altération, notamment :
- chiffrement des mots de passe,
- règles d'accès strictes au niveau de la base de données (chaque utilisateur n'accédant qu'à ses propres données, sauf prérogatives explicitement accordées à un administrateur de Structure ou à l'éditeur),
- accès restreint aux données par les seules personnes habilitées.

## 9. Cookies et traceurs

[À COMPLÉTER selon l'usage réel — si l'application n'utilise que des cookies strictement nécessaires au fonctionnement (session d'authentification), le consentement CNIL n'est en principe pas requis pour ceux-ci ; toute mesure d'audience ou traceur publicitaire supplémentaire devra faire l'objet d'un bandeau de consentement conforme aux recommandations CNIL.]

## 10. Modification de la politique

La présente politique peut être mise à jour. La date de dernière mise à jour figure en bas de ce document. En cas de modification substantielle, les utilisateurs en seront informés.

## 11. Contact

Pour toute question relative à la présente politique de confidentialité : **contact@apezeo.fr**

---

*Dernière mise à jour : [À COMPLÉTER — date de publication]*
`;

export const NON_RESPONSABILITE = `# Clause de non-responsabilité — Apézeo

*Cette clause fait partie intégrante des Conditions Générales d'Utilisation et des Mentions légales d'Apézeo. Elle doit être affichée de manière visible dans l'application (bandeau d'accueil, écran d'inscription) et rappelée dans les documents contractuels signés avec les établissements clients.*

---

## 1. Apézeo n'est ni un médecin, ni un dispositif médical

Apézeo est un outil d'information et de formation destiné à faire connaître des techniques d'accompagnement non médicamenteuses des troubles du comportement liés à la maladie d'Alzheimer et aux maladies apparentées. Il est conçu comme une **aide à la réflexion et à la pratique professionnelle**, et non comme :

- un instrument de diagnostic médical,
- un outil de prescription ou de décision thérapeutique automatisée,
- un substitut à l'évaluation clinique d'un médecin, d'un gériatre, d'un psychiatre ou de tout professionnel de santé habilité,
- un dispositif médical au sens de la réglementation en vigueur (Règlement (UE) 2017/745 relatif aux dispositifs médicaux).

Les fiches proposées par Apézeo constituent des **pistes d'intervention à évaluer par le professionnel** au regard de la situation individuelle de la personne accompagnée, de son histoire de vie, de son état de santé et des prescriptions médicales en cours. Elles ne dispensent en aucun cas d'un avis médical lorsque celui-ci est nécessaire.

## 2. Apézeo ne remplace pas les services d'urgence

**En cas de danger immédiat pour la personne accompagnée ou pour un tiers, l'utilisateur doit immédiatement contacter les services d'urgence compétents** (SAMU - 15, urgences - 112, ou tout numéro d'urgence local applicable), et non chercher une réponse dans l'application.

Apézeo affiche des messages de rappel à cet effet dans son interface, mais ce rappel ne saurait suffire en soi à alerter un utilisateur dans une situation de crise réelle — c'est à l'utilisateur professionnel qu'il incombe d'exercer son jugement clinique et d'appliquer les protocoles d'urgence de son établissement.

## 3. Fiabilité et actualisation du contenu

L'éditeur d'Apézeo s'efforce de proposer un contenu s'appuyant, lorsque cela est indiqué, sur les recommandations de bonnes pratiques professionnelles (HAS, littérature gériatrique, sociétés savantes). Cependant :

- Certaines fiches, identifiées sous la catégorie **« Mes fiches »**, sont explicitement non sourcées et correspondent à des partages d'expérience de terrain, non validés scientifiquement. Elles sont signalées comme telles dans l'application et ne doivent jamais être utilisées comme référence de bonne pratique lors d'un audit ou d'une évaluation qualité.
- L'éditeur ne garantit pas l'exhaustivité, l'actualité permanente ni l'adéquation de chaque fiche à toute situation individuelle.
- Les connaissances scientifiques et les recommandations professionnelles évoluent ; il appartient à l'utilisateur professionnel de maintenir son propre niveau de vigilance et de formation continue au-delà du contenu d'Apézeo.

## 4. Responsabilité de l'utilisateur professionnel

L'utilisateur professionnel demeure seul responsable :

- de l'évaluation de la pertinence d'une technique proposée par Apézeo au regard de la situation réelle de la personne accompagnée,
- du respect des protocoles internes de son établissement et des prescriptions médicales en cours,
- de la décision de mettre en œuvre, adapter ou écarter une intervention suggérée par l'application,
- de la sollicitation d'un avis médical lorsque la situation l'exige.

## 5. Limitation de responsabilité de l'éditeur

Dans les limites permises par la loi applicable, l'éditeur d'Apézeo ne saurait être tenu responsable :

- des conséquences directes ou indirectes résultant de l'application d'une technique proposée dans l'application,
- d'un dommage résultant d'une décision prise sur le seul fondement du contenu d'Apézeo, sans jugement professionnel complémentaire,
- de l'usage qui serait fait d'une fiche « Mes fiches » non sourcée comme s'il s'agissait d'une recommandation validée.

Cette limitation ne s'applique pas en cas de faute lourde ou intentionnelle de l'éditeur, conformément au droit applicable.

## 6. Acceptation

L'utilisation d'Apézeo, notamment la création d'un compte professionnel, vaut acceptation pleine et entière de la présente clause de non-responsabilité.

---

*Dernière mise à jour : [À COMPLÉTER — date de publication]*
*Contact : contact@apezeo.fr*
`;

export const METHODE_EDITORIALE = `# Notre méthode éditoriale — Apézeo

*Comment les fiches d'Apézeo sont construites, sourcées et validées — en toute transparence.*

---

## Deux niveaux, deux logiques

Apézeo propose deux bibliothèques de contenu, construites selon des logiques différentes mais complémentaires.

**La bibliothèque Standard** rassemble plus de 900 fiches pratiques, couvrant un large éventail de techniques d'accompagnement non médicamenteuses, organisées par trouble du comportement et par famille d'intervention.

**La bibliothèque Expert** est construite selon une charte éditoriale stricte, fiche par fiche, avec un niveau d'exigence supérieur en termes de structure, de sourcing et de densité clinique.

---

## Le socle documentaire

Chaque fiche Expert s'appuie sur un nombre restreint de références reconnues, plutôt que sur des sources hétérogènes ou de qualité inégale :

- **Haute Autorité de Santé (HAS)**
- **National Institute for Health and Care Excellence (NICE)**, notamment la recommandation NG97
- **Organisation mondiale de la Santé (OMS)**, notamment le programme ICOPE
- **Société Française de Gériatrie et Gérontologie (SFGG)**
- **France Alzheimer**
- **Fédération des Centres Mémoire (CMRR)**
- **International Psychogeriatric Association (IPA)**

**Règle appliquée systématiquement** : aucune fiche Expert n'est publiée si elle ne peut pas être rattachée à au moins deux références de ce socle documentaire.

**Ce que nous n'utilisons jamais comme source unique** : les blogs, les sites commerciaux, ou les méthodes propriétaires déposées (par exemple Validation®, Humanitude®, Montessori®). Ces approches peuvent être mentionnées lorsque leurs principes sont également décrits dans une recommandation publique reconnue, mais jamais comme unique fondement d'une fiche.

**Ce que nous privilégions** : les recommandations de bonnes pratiques, les consensus d'experts et les guides professionnels — plutôt que des études isolées, qui restent moins robustes à grande échelle.

---

## Une fiche, une compétence

Chaque fiche Expert applique un principe simple : **une fiche décrit un seul geste professionnel précis**, jamais plusieurs techniques mélangées. Une fiche qui expliquerait à la fois comment se présenter, se placer et parler ne respecte pas ce principe — elle doit être découpée en fiches distinctes, chacune consacrée à un seul geste.

Certaines fiches, dites "Concept", font exception : elles présentent un principe général et renvoient explicitement vers les fiches "Technique" qui en détaillent chaque composante — sans jamais les dupliquer.

---

## Ce qu'Apézeo n'est pas

- **Apézeo n'est pas un outil d'aide au diagnostic.** Aucune fiche n'évalue l'état de santé d'une personne précise.
- **Apézeo n'est pas un outil de décision clinique individualisée.** Les fiches proposent des pistes générales à adapter par le professionnel, jamais une prescription pour un patient nommé.
- **Apézeo ne contient aucun arbre décisionnel** qui orienterait automatiquement vers une action en fonction de symptômes saisis pour une personne identifiée.
- **Apézeo ne fait aucune promesse de résultat.** Les techniques proposées sont des pistes à évaluer, pas des garanties.

Voir également notre [Clause de non-responsabilité](#), qui précise ces points en détail.

---

## Un contenu vivant, versionné

Chaque fiche Expert porte un numéro de version. La charte éditoriale elle-même évolue et est versionnée (actuellement V1.0) — toute évolution significative de nos méthodes de rédaction sera documentée ici.

---

*Dernière mise à jour : août 2026*
*Contact : contact@apezeo.fr*
`;
